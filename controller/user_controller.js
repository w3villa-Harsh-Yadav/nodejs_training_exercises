...
const createuser = async(req,res) => {
    try {
        ...
        const token = jwt.sign({username:username,email:email,password:password},process.env.SECRET_KEY);
...
const loginuser = async(req,res) => {
    try {
        ...
        const key = req.body.email+req.body.password;
        ...
        const token = jwt.sign({username:user[0].username,email:user[0].email,password:user[0].password},process.env.SECRET_KEY);
...
const getuser = async(req,res) => {
    try {
        let user = await authenticate(req,cache);
